...
public class EnhancedWindowFinder
{
    ...
    public static string GetProcessPath(Process process)
    {
        try
        {
            ...
            if (!string.IsNullOrEmpty(process.MainModule?.FileName)) return process.MainModule.FileName;
        }
        catch
        {
    ...
    public static List<IntPtr> FindWindowsByProcessPath(Process targetProcess)
    {
        ...
        if (string.IsNullOrEmpty(targetPath)) return new List<IntPtr>();
        ...
        bool EnumWindowCallback(IntPtr hWnd, IntPtr lParam)
        {
            ...
            // 如果这个进程ID已经处理过，跳过
            if (processedPids.Contains(pid)) return true;
                ...
                if (IsWindowVisible(hWnd))
                {
                        ...
                        !className.Contains("Shell_TrayWnd"))
                    {
                        ...
                        if (GetWindowRect(hWnd, ref rect))
                        {
                            ...
                            if (width > 50 && height > 50) windowsList.Add(hWnd);
                        }
                    }
                }
