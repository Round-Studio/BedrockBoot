...
public class ProcessMouseLocker
{
    ...
    public void StartMouseLock()
    {
        ...
        if (_targetWindowHandle == IntPtr.Zero) throw new InvalidOperationException($"无法找到进程ID为 {_processId} 的窗口");
        ...
        while (!_stopEvent.WaitOne(0))
            try
            {
                ...
                if (IsTargetProcessWindow(foregroundWindow) && _targetWindowHandle != IntPtr.Zero)
                {
                    ...
                    {
                        var constrainedPos = ConstrainCursorToSafeArea(cursorPos, _currentWindowRect, BorderMargin);
    ...
    private bool TryGetWindowBounds(IntPtr hWnd, out Rect bounds)
    {
        ...
        try
        {
            var result = DwmGetWindowAttribute(hWnd, DWMWA_EXTENDED_FRAME_BOUNDS, out bounds, Marshal.SizeOf(bounds));
    ...
    {
        var constrainedPos = cursorPos;
        var safeArea = windowRect.GetSafeArea(margin);

        // 水平方向约束到安全区域
        if (cursorPos.x < safeArea.Left)
            constrainedPos.x = safeArea.Left;
        else if (cursorPos.x > safeArea.Right) constrainedPos.x = safeArea.Right;

        // 垂直方向约束到安全区域
        if (cursorPos.y < safeArea.Top)
            constrainedPos.y = safeArea.Top;
        else if (cursorPos.y > safeArea.Bottom) constrainedPos.y = safeArea.Bottom;
    ...
    private void FindTargetWindow()
    {
        ...
        EnumWindows((hWnd, lParam) =>
        {
            ...
            if (processId == _processId)
            {
                ...
                if (IsSuitableWindow(windowInfo))
                {
                    ...
                    Console.WriteLine(
                        $@"找到候选窗口: 句柄={hWnd}, 进程ID={processId}, 标题={windowInfo.Title}, 时间戳={windowInfo.Timestamp}, 差值={windowInfo.TimestampDifference}");
                }
            }
        ...
        if (_targetWindowHandle != IntPtr.Zero) Console.WriteLine($@"已选择目标窗口: 句柄={_targetWindowHandle}");
    }
    ...
    private WindowInfo GetWindowInfo(IntPtr hWnd, uint processId)
    {
        ...
        // 获取窗口类名
        var classNameBuilder = new StringBuilder(MAX_CLASS_NAME);
        GetClassName(hWnd, classNameBuilder, MAX_CLASS_NAME);
        windowInfo.ClassName = classNameBuilder.ToString();

        // 获取窗口标题
        var windowTextBuilder = new StringBuilder(MAX_WINDOW_TEXT);
