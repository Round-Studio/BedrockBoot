...
namespace PEFileValidator
{
    public class PEFileValidator64
    {
        ...
        public static bool IsValidPEFile64(string filePath)
        {
            try
            {
                ...
                using (var br = new BinaryReader(fs))
                {
                    ...
                    {
                        Console.WriteLine($"机器类型不是64位: 0x{fileHeader.Machine:X4} (期望: 0x{IMAGE_FILE_MACHINE_AMD64:X4}) - {filePath}");
                        return false;
                    }
                    ...
                    {
                        Console.WriteLine($"可选头魔数不是64位: 0x{optionalHeaderMagic:X4} (期望: 0x{NT_OPTIONAL_64_MAGIC:X4}) - {filePath}");
                        return false;
                    }
        ...
        private static IMAGE_DOS_HEADER ReadDosHeader(BinaryReader br)
        {
            ...
            try
            {
                var header = (IMAGE_DOS_HEADER)Marshal.PtrToStructure(handle.AddrOfPinnedObject(), typeof(IMAGE_DOS_HEADER));
                return header;
            }
            finally
            {
        ...
        private static IMAGE_FILE_HEADER ReadFileHeader(BinaryReader br)
        {
            ...
            try
            {
                var header = (IMAGE_FILE_HEADER)Marshal.PtrToStructure(handle.AddrOfPinnedObject(), typeof(IMAGE_FILE_HEADER));
                return header;
            }
            finally
            {
        ...
        private static IMAGE_OPTIONAL_HEADER64 ReadOptionalHeader64(BinaryReader br)
        {
            ...
            try
            {
                var header = (IMAGE_OPTIONAL_HEADER64)Marshal.PtrToStructure(handle.AddrOfPinnedObject(), typeof(IMAGE_OPTIONAL_HEADER64));
                return header;
            }
            finally
            {
        ...
        private static bool ValidateSections(BinaryReader br, ushort numberOfSections, int fileSize)
        {
            for (int i = 0; i < numberOfSections; i++)
            {
                ...
                try
                {
                    var sectionHeader = (IMAGE_SECTION_HEADER)Marshal.PtrToStructure(handle.AddrOfPinnedObject(), typeof(IMAGE_SECTION_HEADER));

                    if (sectionHeader.PointerToRawData != 0 && sectionHeader.PointerToRawData + sectionHeader.SizeOfRawData > fileSize)
                    {
                        Console.WriteLine($"区段数据超出文件范围: 偏移={sectionHeader.PointerToRawData}, 大小={sectionHeader.SizeOfRawData}, 文件大小={fileSize}");
                        return false;
                    }
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            string[] testFiles = {
                ...
                @"D:\BedrockBoot\bedrock_versions\无效实例\Minecraft.Windows.exe"
            };

            foreach (var exePath in testFiles)
            {
                Console.WriteLine($"\n正在验证: {exePath}");
                
                bool isValid = PEFileValidator64.IsValidPEFile64(exePath);
                Console.WriteLine($"64位PE文件有效性: {(isValid ? "有效" : "无效")}");
                
                if (!isValid)
                {
                    Console.WriteLine("获取详细PE信息:");
                    Console.WriteLine(PEFileValidator64.GetPEFileInfo(exePath));
                }
            }
