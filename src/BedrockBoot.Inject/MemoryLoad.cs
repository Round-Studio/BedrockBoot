...
public class MemoryModule : IDisposable
{
	...
	private DllEntryDelegate _dllEntry = null;
	private ExeEntryDelegate _exeEntry = null;
	...
	public TDelegate GetDelegateFromFuncName<TDelegate>(string funcName) where TDelegate : class
	{
		...
		TDelegate res = Marshal.GetDelegateForFunctionPointer<TDelegate>(GetPtrFromFuncName(funcName));
	...
	private nint GetPtrFromFuncName(string funcName)
	{
		...
		nint pDirectory = PtrAdd(_pNtHeaders, Of.ImageNtHeadersOptionalHeader + (Is64BitProcess ? Of64.ImageOptionalHeaderExportTable : Of32.ImageOptionalHeaderExportTable));
		...
		nint pExports = PtrAdd(_pCode, directory.VirtualAddress);
		...
		nint pNameRef = PtrAdd(_pCode, exports.AddressOfNames);
		nint pOrdinal = PtrAdd(_pCode, exports.AddressOfNameOrdinals);
		for (int i = 0; i < exports.NumberOfNames; i++, pNameRef = PtrAdd(pNameRef, sizeof(uint)), pOrdinal = PtrAdd(pOrdinal, sizeof(ushort)))
		{
			...
			if (curFuncName == funcName)
			{
				...
				nint pAddressOfFunction = PtrAdd(_pCode, (exports.AddressOfFunctions + (uint)(ordinal * 4)));
	...
	private void MemoryLoadLibrary(byte[] data)
	{
		...
		if (Is64BitProcess) oldHeaderOptionalHeaderImageBase = (nint)unchecked((long)(orgNtHeaders.OptionalHeader.ImageBaseLong));
		else oldHeaderOptionalHeaderImageBase = unchecked((int)(orgNtHeaders.OptionalHeader.ImageBaseLong >> 32));

		// reserve memory for image of library
		_pCode = Win.VirtualAlloc(oldHeaderOptionalHeaderImageBase, orgNtHeaders.OptionalHeader.SizeOfImage, AllocationType.Reserve | AllocationType.Commit, MemoryProtection.Readwrite);
		//pCode = IntPtr.Zero; //test relocation with this

		// try to allocate memory at arbitrary position
		if (_pCode == nint.Zero) _pCode = Win.VirtualAlloc(nint.Zero, orgNtHeaders.OptionalHeader.SizeOfImage, AllocationType.Reserve | AllocationType.Commit, MemoryProtection.Readwrite);
		...
		if (Is64BitProcess && PtrSpanBoundary(_pCode, alignedImageSize, 32))
		{
			...
			System.Collections.Generic.List<nint> blockedMemory = new System.Collections.Generic.List<nint>();
			while (PtrSpanBoundary(_pCode, alignedImageSize, 32))
			{
				blockedMemory.Add(_pCode);
				_pCode = Win.VirtualAlloc(nint.Zero, alignedImageSize, AllocationType.Reserve | AllocationType.Commit, MemoryProtection.Readwrite);
				if (_pCode == nint.Zero) break;
			}
			foreach (nint ptr in blockedMemory) Win.VirtualFree(ptr, nint.Zero, AllocationType.Release);
			if (_pCode == nint.Zero) throw new DllException("Out of Memory");
		}

		// commit memory for headers
		nint headers = Win.VirtualAlloc(_pCode, orgNtHeaders.OptionalHeader.SizeOfHeaders, AllocationType.Commit, MemoryProtection.Readwrite);
		...
		nint locationDelta = PtrSub(_pCode, oldHeaderOptionalHeaderImageBase);
		if (locationDelta != nint.Zero)
		{
			...
			nint pImageBase = PtrAdd(_pNtHeaders, Of.ImageNtHeadersOptionalHeader + (Is64BitProcess ? Of64.ImageOptionalHeaderImageBase : Of32.ImageOptionalHeaderImageBase));
		...
		_isRelocated = (locationDelta != nint.Zero ? PerformBaseRelocation(ref orgNtHeaders, _pCode, locationDelta) : true);
		...
		if (orgNtHeaders.OptionalHeader.AddressOfEntryPoint != 0)
		{
			if (IsDll)
			{
				...
				nint dllEntryPtr = PtrAdd(_pCode, orgNtHeaders.OptionalHeader.AddressOfEntryPoint);
			...
			{
				nint exeEntryPtr = PtrAdd(_pCode, orgNtHeaders.OptionalHeader.AddressOfEntryPoint);
	...
	{
		nint pSection = Win.IMAGE_FIRST_SECTION(pNtHeaders, orgNtHeaders.FileHeader.SizeOfOptionalHeader);
		for (int i = 0; i < orgNtHeaders.FileHeader.NumberOfSections; i++, pSection = PtrAdd(pSection, Sz.ImageSectionHeader))
		{
			...
			if (section.SizeOfRawData == 0)
			{
				...
				if (size > 0)
				{
					nint dest = Win.VirtualAlloc(PtrAdd(pCode, section.VirtualAddress), size, AllocationType.Commit, MemoryProtection.Readwrite);
			...
			else
			{
				...
				nint dest = Win.VirtualAlloc(PtrAdd(pCode, section.VirtualAddress), section.SizeOfRawData, AllocationType.Commit, MemoryProtection.Readwrite);
	...
	private static bool PerformBaseRelocation(ref ImageNtHeaders orgNtHeaders, nint pCode, nint delta)
	{
		...
		for (nint pRelocation = PtrAdd(pCode, orgNtHeaders.OptionalHeader.BaseRelocationTable.VirtualAddress); ;)
		{
			ImageBaseRelocation relocation = PtrRead<ImageBaseRelocation>(pRelocation);
			if (relocation.VirtualAdress == 0) break;

			nint pDest = PtrAdd(pCode, relocation.VirtualAdress);
			nint pRelInfo = PtrAdd(pRelocation, Sz.ImageBaseRelocation);
			...
			for (uint i = 0; i != relCount; i++, pRelInfo = PtrAdd(pRelInfo, sizeof(ushort)))
			{
				...
				nint pPatchAddr = PtrAdd(pDest, offset);
	...
	{
		System.Collections.Generic.List<nint> importModules = new System.Collections.Generic.List<nint>();
		uint numEntries = orgNtHeaders.OptionalHeader.ImportTable.Size / Sz.ImageImportDescriptor;
		nint pImportDesc = PtrAdd(pCode, orgNtHeaders.OptionalHeader.ImportTable.VirtualAddress);
		for (uint i = 0; i != numEntries; i++, pImportDesc = PtrAdd(pImportDesc, Sz.ImageImportDescriptor))
		{
			...
			nint handle = Win.LoadLibrary(PtrAdd(pCode, importDesc.Name));
			if (PtrIsInvalidHandle(handle))
			{
				foreach (nint m in importModules) Win.FreeLibrary(m);
				importModules.Clear();
			...
			for (int szRef = nint.Size; ; pThunkRef = PtrAdd(pThunkRef, szRef), pFuncRef = PtrAdd(pFuncRef, szRef))
			{
	...
	{
		nuint imageOffset = (Is64BitProcess ? (nuint)(unchecked((ulong)pCode.ToInt64()) & 0xffffffff00000000) : nuint.Zero);
		nint pSection = Win.IMAGE_FIRST_SECTION(pNtHeaders, orgNtHeaders.FileHeader.SizeOfOptionalHeader);
		ImageSectionHeader section = PtrRead<ImageSectionHeader>(pSection);
		...
		for (int i = 1; i < orgNtHeaders.FileHeader.NumberOfSections; i++, pSection = PtrAdd(pSection, Sz.ImageSectionHeader))
		{
			...
			nint sectionAddress = PtrBitOr(PtrAdd(0, section.PhysicalAddress), imageOffset);
			nint alignedAddress = PtrAlignDown(sectionAddress, pageSize);
			nint sectionSize = GetRealSectionSize(ref section, ref orgNtHeaders);
			...
			nint a = PtrAdd(sectionData.Address, sectionData.Size);
	...
	private static void FinalizeSection(SectionFinalizeData sectionData, uint pageSize, uint sectionAlignment)
	{
		...
		uint protect = (uint)_protectionFlags[executable, readable, writeable];
	...
	private static void ExecuteTls(ref ImageNtHeaders orgNtHeaders, nint pCode, nint pNtHeaders)
	{
		...
		nint pCallBack = tlsDir.AddressOfCallBacks;
		if (pCallBack != nint.Zero)
		{
			for (nint callback; (callback = PtrRead<nint>(pCallBack)) != nint.Zero; pCallBack = PtrAdd(pCallBack, nint.Size))
			{
	...
	public void Dispose()
	{
		...
		{
			foreach (nint m in _importModules) if (!PtrIsInvalidHandle(m)) Win.FreeLibrary(m);
			_importModules = null;
		}
	...
	private class Win
	{
		...
		public static extern nint VirtualAlloc(nint lpAddress, nuint dwSize, AllocationType flAllocationType, MemoryProtection flProtect);
		...
		[DllImport("kernel32.dll", SetLastError = true)]
		public static extern bool VirtualProtect(nint lpAddress, nint dwSize, uint flNewProtect, out uint lpflOldProtect);
	...
	{ return (nint.Size == 8 ? (nint)(p.ToInt64() + unchecked((long)v.ToUInt64())) : p.ToInt32() + unchecked((int)v.ToUInt32())); }
	private static nint PtrSub(nint p, nint v)
	{ return (nint.Size == 8 ? (nint)(p.ToInt64() - v.ToInt64()) : p.ToInt32() - v.ToInt32()); }
	private static nint PtrBitOr(nint p, nuint v)
	{ return (nint.Size == 8 ? (nint)unchecked((long)(unchecked((ulong)p.ToInt64()) | v.ToUInt64())) : unchecked((int)(unchecked((uint)p.ToInt32()) | v.ToUInt32()))); }
