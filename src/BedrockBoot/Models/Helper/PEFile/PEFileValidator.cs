...
public class PEFileValidator
{
    ...
    public static bool IsValidPEFile64(string filePath)
    {
        try
        {
            ...
            using (var br = new BinaryReader(fs))
            {
                ...
                var e_lfanew = dosHeader.e_lfanew;
    ...
    public static bool IsValidEffectivePEFile(string filePath)
    {
        try
        {
            ...
            using (var br = new BinaryReader(fs))
            {
                ...
                if (dosHeader.e_magic != DOS_SIGNATURE) return false;

                var e_lfanew = dosHeader.e_lfanew;

                if (e_lfanew < 0x40 || e_lfanew >= fs.Length) return false;

                fs.Seek(e_lfanew, SeekOrigin.Begin);

                var peSignature = br.ReadUInt32();
                if (peSignature != PE_SIGNATURE) return false;
    ...
    private static IMAGE_DOS_HEADER ReadDosHeader(BinaryReader br)
    {
        ...
        if (buffer.Length != 64) throw new IOException("无法读取完整的DOS头");
    ...
    public static string GetPEFileInfo(string filePath)
    {
        try
        {
            ...
            using (var br = new BinaryReader(fs))
            {
                ...
                if (dosHeader.e_magic != DOS_SIGNATURE) return "不是有效的PE文件 (DOS签名无效)";

                var e_lfanew = dosHeader.e_lfanew;
                // Console.WriteLine($"Debug: GetPEFileInfo e_lfanew: {e_lfanew} (0x{e_lfanew:X8})"); // 调试输出

                if (e_lfanew < 0x40 || e_lfanew >= fs.Length) return "不是有效的PE文件 (PE头偏移无效)";

                fs.Seek(e_lfanew, SeekOrigin.Begin);

                var peSignature = br.ReadUInt32();
                if (peSignature != PE_SIGNATURE) return "不是有效的PE文件 (PE签名无效)";
