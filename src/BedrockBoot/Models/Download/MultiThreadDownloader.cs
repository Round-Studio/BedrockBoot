...
public class MultiThreadDownloader : IDisposable
{
    ...
    public MultiThreadDownloader(int maxConcurrency = 4, int bufferSize = 81920, int defaultTimeoutSeconds = 20)
    {
        ...
        {
            PooledConnectionLifetime = TimeSpan.FromMinutes(2), // 连接2分钟后重建
            PooledConnectionIdleTimeout = TimeSpan.FromMinutes(1),
            MaxConnectionsPerServer = maxConcurrency * 2, // 为每个连接留出余量
    ...
    {
        // 动态计算合理的分段数量
        // var actualParts = CalculateOptimalParts(fileSize);
        ...
        void ReportProgressIfNeeded(bool force = false)
        {
            ...
            var shouldReport = force || (now - lastReportTime) >= ProgressReportInterval;
        ...
        try
        {
            ...
            for (int i = 0; i < parts.Count; i++)
            {
                ...
                {
                    await DownloadPartWithRetryAsync(uri, part.Start, part.End, tempFilePath,
                        i, downloadInfo, totalDownloadedBytes, ReportProgressIfNeeded, cancellationToken);
                }, cancellationToken));
            }
    ...
    private async System.Threading.Tasks.Task MergeTempFilesAsync(string[] tempFiles, string outputPath, CancellationToken cancellationToken)
    {
        ...
        await Parallel.ForEachAsync(sortedFiles, parallelOptions, async (fileInfo, ct) =>
        {
